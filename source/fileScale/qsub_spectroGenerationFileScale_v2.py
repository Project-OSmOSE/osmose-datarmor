...
def generate_and_save_figures(colmapspectros,segment_times, Freq, log_spectro,min_color_val,max_color_val,output_file):
...
def gen_spectro(max_w, min_color_val, data, sample_rate,  win_size, nfft, pct_overlap,duration,output_file):
    ...
    for idwin in range(Nbwin):
        ...
        if Nfft < (0.5*Nwin):
            _, Sxx[:,idwin] = signal.welch(x_win, fs=fs, window='hanning', nperseg = Nfft, noverlap = int(Nfft/2))#, scaling='spectrum')
        else :
    ...
    segment_times = np.linspace(0,duration,Sxx.shape[1])#np.arange(win_size / 2, x.shape[-1] - win_size / 2 + 1, win_size - Noverlap) / float(sample_rate)
        
               
        
    print('DIM LOG SPECTRO:',log_spectro.shape)
    generate_and_save_figures(colmapspectros,segment_times, Freq, log_spectro,min_color_val,max_color_val,output_file)
...
def gen_tiles(nber_level,data, sample_rate, output,winsize, nfft, overlap,min_color_val,segment_str):  
    ...
    for tile in range(0, nber_tiles_lowest_zoom_level):
        ...
        Sxx,Freq = gen_spectro(max_w, min_color_val, sample_data, sample_rate, winsize,nfft,overlap,len(sample_data)/sample_rate,output_file)
    ...
    for ll in range(nber_level)[::-1]:
        ...
        for kk in range(2**ll):
            ...
            generate_and_save_figures(colmapspectros,segment_times_int, Freq, log_spectro,min_color_val,max_color_val,output_file)
...
if __name__ == "__main__":
    ...
    if am_local:
        ...
        #dataset_ID
        analysis_fs = 240
        dataset_ID = 'gliderSPAms_TP_SOIA'

        # analysis parameters TO BE DEFINED
