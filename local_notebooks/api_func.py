...
def save_results(dates, lat, lon, single_levels, variables):
	for i in range(len(variables)):
		np.save(variables[i], np.ma.filled(single_levels[i], fill_value=float('nan')), allow_pickle = True)
	stamps = np.zeros((len(dates), len(lat), len(lon), 3), dtype=object)
	for i in range(len(dates)):
		for j in range(len(lat)):
			for k in range(len(lon)):
				stamps[i,j,k] = [dates[i], lat[j], lon[k]]
	np.save('stamps', stamps, allow_pickle = True)
	print('\n[============]\nDone')
...
def final_creation(df1, filename, key, variable, year, month, day, time, area, type_crea = 'complexe') :
  ...
  test = save_results(dates, lat, lon, single_levels, variables)
  return test
...
def mise_en_forme_old(filename):
  ...
  variables = list(fh.variables.keys()[3:])
  single_levels = np.zeros((len(variables), fh.dimensions.get('longitude').size, fh.dimensions.get('longitude').size, fh.dimensions.get('longitude').size))
